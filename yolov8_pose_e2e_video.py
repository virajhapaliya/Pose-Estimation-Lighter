...
def _draw_skeleton(frame, scaled_nms_output):
    ...
    for result in scaled_nms_output:
        ...
        # Extract the box coordinates and dimensions
        half_w = (box[2] / 2)
        half_h = (box[3] / 2)
        x0 = int(box[0] - half_w)
        y0 = int(box[1] - half_h)
        ...
        cv2.rectangle(frame, (x0, y0), (x1, y1), (0, 0, 255), 2)  # red rectangle, thickness = 2

        # Loop through the skeleton connections
        for sk in skeleton:
            ...
            if conf1 < 0.5 or conf2 < 0.5:
                continue  # Skip if confidence is below 0.5

            # Check if the coordinates are within the frame dimensions
            ...
            if coord_valid(pos1) and coord_valid(pos2):
                cv2.line(frame, pos1, pos2, (0, 255, 255), 2)  # yellow line, thickness = 2

    return frame


def _run_inference(onnx_model_path: Path, model_input: str, model_outputs_image: bool, test_image: Path,
                   rgb_layout: Optional[str],video_path: Optional[str] = None):
    ...
    if video_path:
        ...
        while cap.isOpened():
            ...
            # Optionally, you can also convert it to a numpy array with uint8 (similar to image_bytes)
            
            frame_bytes_array = np.frombuffer(frame_bytes, dtype=np.uint8)


            # Prepare the input for the model
            model_input_data = {input_name[0]: frame_bytes_array}
            # Convert frame to bytes

            model_output = ['image'] if model_outputs_image else ['nms_output_with_scaled_boxes_and_keypoints']
            start_time = time()
            outputs = session.run(model_output, model_input_data)
            elapsed_time = time() - start_time

            fps = 1 / elapsed_time

            # print("Total_time : ",elapsed_time)
            # For video, let's assume the model returns an image with bounding boxes drawn
            if model_outputs_image:
                ...
                out.write(img_bgr)  # Write frame to output video
            else:
                # Add bounding boxes and skeletons manually (same as in the image function)
                frame =  _draw_skeleton(frame, outputs[0])
                out.write(frame)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--input", choices=("image", "rgb","video"), default="image",
    ...
    parser.add_argument("--video", type=str, required=False,
                        help="Path to video file for video processing.")
    ...
    if args.input == "video" and args.video:
        ...
        rgb_layout = None
        _run_inference(updated_model_path, args.input, args.output_image is not None, None, None, args.video)
    else:
        if args.input == "rgb":
        ...
        if args.run_model:
            print("Updating model with Run model as input.")
